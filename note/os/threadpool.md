...
上述线程池的实现是最简单的方法，将任务放在任务队列中，然后线程池的线程依次消费任务队列。我们使用两个线程处理队列中的任务，一个负责添加任务到队列，一个负责消费任务队列。

如果在某一时刻突然任务激增，那么只用两个线程足够吗？如果长时间任务队列为空，我们还保留这两个线程吗？

在添加任务时我们需要对任务队列加锁，以防止多个任务同时添加。在有任务需要执行时，工作线程去任务队列获取任务执行，这一过程也需要加锁，其中还涉及了任务队列为空时的等待处理。

我们设想一种情况，此刻 pool 中一共有 5 个 thread 可以运行，而 queue 中有3个任务，分别是 Task0，Task1，Task2 吧。会发生什么，这 5 个 thread 去争抢 Task0 的执行权，其中有一个拿到了，满意的去执行了。然后接下来 4 个 thread 再去争抢 Task1 任务的执行权，一个成功之后，剩下的 3 个 thread 再去争抢 Task2 的执行权，over。

 这里说“争抢”，其实就是**抢锁**的一个过程，因为从 queue 中获取任务是需要上锁的，这个过程是有等待损耗的。而且，理论上这种 `one by one` 的同步，相对是很耗时的。 

所有提升并发的优化，都有两个最基本的出发点：**增加扇入扇出，增加负载**。也就是批量进出，批量处理。